...
var ParamSets = params.Sets{
	{Name: "Base", Desc: "these are the best params", Sheets: params.Sheets{
		...
		"Network": &params.Sheet{
			...
			{Sel: ".Target", Desc: "output definitely needs lower inhib -- true for smaller layers in general",
				Params: params.Params{
	...
	{Name: "DefaultInhib", Desc: "output uses default inhib instead of lower", Sheets: params.Sheets{
		"Network": &params.Sheet{
			{Sel: ".Target", Desc: "go back to default",
				Params: params.Params{
					"Layer.Inhib.Layer.Gi": "1.8",
				}},
		},
...
type Sim struct {
	...
	LesionProp float32 `desc:"proportion of neurons lesioned -- use Lesion button to lesion"`
...
func (ss *Sim) New() {
	...
	ss.PCAInterval = 5
	ss.Time.Defaults()
	ss.LayStatNms = []string{"Item", "Relation", "Representation", "Hidden", "Class", "Characteristics", "Adjective", "Verb"}
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	ss.Params.AddLayers([]string{"Representation", "Hidden"}, "Hidden")
	ss.Params.SetObject("NetSize")

	net.InitName(net, "RA25")
	inp1 := net.AddLayer2D("Item", 1, 8, emer.Input)
	inp2 := net.AddLayer2D("Relation", 1, 4, emer.Input)
	hid1 := net.AddLayer2D("Representation", 1, 8, emer.Hidden)
	hid2 := net.AddLayer2D("Hidden", 7, 7, emer.Hidden)
	out1 := net.AddLayer2D("Class", 1, 15, emer.Target)
	out2 := net.AddLayer2D("Adjective", 1, 6, emer.Target)
	out3 := net.AddLayer2D("Verb", 1, 5, emer.Target)
	out4 := net.AddLayer2D("Characteristics", 1, 8, emer.Target)
	...
	net.BidirConnectLayers(hid2, out1, full)
	net.BidirConnectLayers(hid2, out2, full)
	net.BidirConnectLayers(hid2, out3, full)
	net.BidirConnectLayers(hid2, out4, full)
...
func (ss *Sim) LesionNet(net *leabra.Network, prop float32) {
	prop = ss.LesionProp
	net.LayerByName("Hidden").(leabra.LeabraLayer).AsLeabra().LesionNeurons(prop)
}
...
func (ss *Sim) Init() {
	...
	ss.NewRun()
	ss.ViewUpdt.Update()
	ss.LesionNet(ss.Net, ss.LesionProp)
}
...
func (ss *Sim) ApplyInputs(en env.Env) {
	...
	lays := []string{"Item", "Relation", "Class", "Adjective", "Verb", "Characteristics"}
...
func (ss *Sim) ConfigPats() {
	...
	sch := etable.Schema{
		...
		{"Item", etensor.FLOAT32, []int{1, 8}, []string{"Y", "X"}},
		{"Relation", etensor.FLOAT32, []int{1, 4}, []string{"Y", "X"}},
		{"Class", etensor.FLOAT32, []int{1, 15}, []string{"Y", "X"}},
		{"Adjective", etensor.FLOAT32, []int{1, 6}, []string{"Y", "X"}},
		{"Verb", etensor.FLOAT32, []int{1, 5}, []string{"Y", "X"}},
		{"Characteristics", etensor.FLOAT32, []int{1, 8}, []string{"Y", "X"}},
	}
...
func (ss *Sim) OpenPats() {
	...
	dt.SetMetaData("name", "TrainPats")
	dt.SetMetaData("desc", "Training patterns")
	err := dt.OpenCSV("SemanticRepresentation.tsv", etable.Tab)
	if err != nil {
		log.Println(err)
	}
}
...
func (ss *Sim) TrialStats() {
	out := ss.Net.LayerByName("Class").(leabra.LeabraLayer).AsLeabra()
...
func (ss *Sim) LogTstTrl(dt *etable.Table) {
	...
	inp := ss.Net.LayerByName("Item").(leabra.LeabraLayer).AsLeabra()
	hid := ss.Net.LayerByName("Hidden").(leabra.LeabraLayer).AsLeabra()
	out1 := ss.Net.LayerByName("Class").(leabra.LeabraLayer).AsLeabra()
	out2 := ss.Net.LayerByName("Verb").(leabra.LeabraLayer).AsLeabra()
	out3 := ss.Net.LayerByName("Adjective").(leabra.LeabraLayer).AsLeabra()
	out4 := ss.Net.LayerByName("Characteristics").(leabra.LeabraLayer).AsLeabra()
	...
	ivt := ss.ValsTsr("Item")
	hdt := ss.ValsTsr("Hidden")
	ov1 := ss.ValsTsr("Class")
	ov2 := ss.ValsTsr("Verb")
	ov3 := ss.ValsTsr("Adjective")
	ov4 := ss.ValsTsr("Characteristics")
	
	inp.UnitValsTensor(ivt, "Act")
	dt.SetCellTensor("ItemAct", row, ivt)

	hid.UnitValsTensor(hdt, "Hid")
	dt.SetCellTensor("HiddenAct", row, hdt)
	
	out1.UnitValsTensor(ov1, "ActM")
	dt.SetCellTensor("ClassActM", row, ov1)
	
	out2.UnitValsTensor(ov2, "ActM")
	dt.SetCellTensor("VerbActM", row, ov2)
	
	out3.UnitValsTensor(ov3, "ActM")
	dt.SetCellTensor("AdjActM", row, ov3)

	out4.UnitValsTensor(ov4, "ActM")
	dt.SetCellTensor("CharActM", row, ov4)
...
func (ss *Sim) ConfigTstTrlLog(dt *etable.Table) {
	...
	sch = append(sch, etable.Schema{
		{"ItemAct", etensor.FLOAT64, inp.Shp.Shp, nil},
		{"HiddenAct", etensor.FLOAT64, hid.Shp.Shp, nil},
		{"ClassActM", etensor.FLOAT64, out1.Shp.Shp, nil},
		{"VerbActM", etensor.FLOAT64, out2.Shp.Shp, nil},
		{"AdjActM", etensor.FLOAT64, out3.Shp.Shp, nil},
		{"CharActM", etensor.FLOAT64, out4.Shp.Shp, nil},
	}...)
...
func (ss *Sim) ConfigTstTrlPlot(plt *eplot.Plot2D, dt *etable.Table) *eplot.Plot2D {
	...
	plt.SetColParams("ItemAct", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("HiddenAct", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("ClassActM", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("VerbActM", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("AdjActM", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
	plt.SetColParams("CharActM", eplot.Off, eplot.FixMin, 0, eplot.FixMax, 1)
	return plt
}
...
func (ss *Sim) RepsAnalysis() {
	...
	ss.ClustPlot(ss.HiddenReps.ClustPlot, sorted, "HiddenAct")
	ss.Stopped()
}
...
func (ss *Sim) ConfigGui() *gi.Window {
	title := "Semantic Representation Network"
